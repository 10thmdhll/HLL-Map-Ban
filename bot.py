...
def create_ban_status_image(
    ...
    part1 = f"Flip Winner: {flip_lbl}"
    part2 = f"First Ban: {first_lbl}"
    part3 = host_field
    ...
    draw.text((total_w//2,y+banner_h//2),
              f"{part1} | {part2} | {part3}",
              font=hdr_font,anchor="mm",fill="black")
    y += banner_h
    ...
    for w,l in zip(cols,labels):
        ...
        if l: draw.text((x+w//2,y+h2//2),l,font=hdr_font,anchor="mm",fill="black")
        x+=w
    y += h2
...
async def side_autocomplete(interaction:discord.Interaction,current:str):
    ch=interaction.channel_id
    if ch not in ongoing_bans: return []
    sel=interaction.namespace.map_name; team=match_turns.get(ch); opts=[]
...
async def match_create(
    ...
    if ch in ongoing_bans:
        return await interaction.response.send_message("❌ Match active; delete first.",ephemeral=True)
    ...
    pairing=determine_ban_option(ra,rb,cfg)  # "ExtraBan"/"DetermineHost"

    # coin flip
    winner_key=random.choice(["team_a","team_b"])
    channel_flip[ch]=winner_key
    channel_mode[ch]=pairing
    ...
    img=create_ban_status_image(maps,ongoing_bans[ch],a,b,
                                pairing,flip_lbl,channel_decision[ch],cur_lbl)
...
async def ban_map(
    ...
    if channel_mode.get(ch)=="DetermineHost" and channel_decision.get(ch) is None:
        return await interaction.followup.send("❌ Waiting for flip-winner decision; use `/match_decide`.",ephemeral=True)
    ...
    if len(combos_pre)==2 and combos_pre[0][0]==combos_pre[1][0]:
        ...
        img=create_ban_status_image(load_maplist(),ongoing_bans[ch],
                                    ...
                                    channel_mode[ch],flip_lbl,channel_decision[ch],None)
    ...
    img=create_ban_status_image(load_maplist(),ongoing_bans[ch],
                                channel_teams[ch][0],channel_teams[ch][1],
                                channel_mode[ch],flip_lbl,channel_decision[ch],cur_lbl)
...
async def match_decide(
    ...
    winner_lbl=channel_teams[ch][0] if winner_key=="team_a" else channel_teams[ch][1]
    if winner_lbl not in [r.name for r in interaction.user.roles]:
        return await interaction.response.send_message("❌ Only flip winner can decide.",ephemeral=True)
    ...
    img=create_ban_status_image(load_maplist(),ongoing_bans[ch],
                                a_lbl,b_lbl,
                                channel_mode[ch],
                                a_lbl if winner_key=="team_a" else b_lbl,
                                choice,
                                a_lbl if match_turns[ch]=="team_a" else b_lbl)
    await update_status_message(ch,None,img)
    await interaction.response.send_message(
        f"✅ You chose **{'First Ban' if choice=='ban' else 'Host'}**; first ban: **{a_lbl if match_turns[ch]=='team_a' else b_lbl}**.",
        ephemeral=True
    )
