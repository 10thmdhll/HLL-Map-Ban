...
channel_messages: dict[int, int] = {}           # channel_id → message_id of the status image
    ...
    if msg_id:
        ...
        if channel:
            try: 
                ...
                discord.utils.suppress(Exception)(msg.delete)()
            except:
                pass
    save_state()
...
def create_ban_status_image(
    ...
    if current_turn_label:
        ...
        draw.text((total_w//2, y+banner_h//2),f"Current Turn: {current_turn_label}",
    ...
    # rows
    for m in map_list:
        name=m["name"]; tb=bans.get(name,{"team_a":{"manual":[],"auto":[]},"team_b":{"manual":[],"auto":[]}})
        x=0
        for team in ("team_a","team_b"):
            for side in ("Allied","Axis"):
                is_final=final_combo and (name,team,side) in final_combo
                if is_final: c=(180,255,180)
                elif side in tb[team]["manual"]: c=(255,0,0)
                elif side in tb[team]["auto"]:   c=(255,165,0)
                else: c=(255,255,255)
...
async def side_autocomplete(interaction:discord.Interaction,current:str):
    ...
    team=match_turns[ch]; tb=ongoing_bans[ch][selected][team]; opts=[]
    for s in ("Allied","Axis"):
...
async def match_create(interaction:discord.Interaction,
    ...
    if ch in ongoing_bans:
        return await interaction.response.send_message(
            "❌ Match active; delete first.",ephemeral=True)
    ...
    match_turns[ch]="team_a";channel_teams[ch]=(a,b)
    save_state()
    current=a
...
async def ban_map(interaction:discord.Interaction,
    ...
    if ch not in ongoing_bans:
        return await interaction.response.send_message(
            "❌ No match; create one.",ephemeral=True)
    # check final
    combos_pre=[(n,t,s)
    ...
    if len(combos_pre)==2 and combos_pre[0][0]==combos_pre[1][0]:
        ...
        msg_id=channel_messages[ch]
        channel=bot.get_channel(ch)
        msg=await channel.fetch_message(msg_id)
        await msg.edit(content=content,attachments=[discord.File(img)])
        return
    # apply ban
    tk=match_turns[ch];ok="team_b" if tk=="team_a" else "team_a"
    tb=ongoing_bans[ch][map_name]
    tb[tk]["manual"].append(side)
    opp="Axis" if side=="Allied" else "Allied"
    tb[ok]["auto"].append(opp)
    match_turns[ch]=ok;save_state()
    a_label,b_label=channel_teams[ch]
    ...
    msg_id=channel_messages[ch];channel=bot.get_channel(ch)
    msg=await channel.fetch_message(msg_id)
    await msg.edit(content=None,attachments=[discord.File(img)])
...
async def match_delete(interaction:discord.Interaction):
    ch=interaction.channel_id
    if ch not in ongoing_bans:
        return await interaction.response.send_message("❌ No match.",ephemeral=True)
    cleanup_match(ch)
...
async def on_ready():
    load_state()
    await bot.tree.sync()
    print("Ready; channels:",list(ongoing_bans.keys()))
