...
def create_ban_status_image(
    ...
    def measure(fnt: ImageFont.FreeTypeFont, txt: str) -> Tuple[int, int]:
        bbox = fnt.getbbox(txt)
        return bbox[2] - bbox[0], bbox[3] - bbox[1]

    # measure sides and maps
    ...
    # base side width
    base_side_w = max(max_side_w, measure(hdr_font, "Allied")[0]) + pad_x * 2
    # ensure two side columns can fit longest team name
    t_a_w, _ = measure(hdr_font, team_a_label)
    t_b_w, _ = measure(hdr_font, team_b_label)
    max_team_w = max(t_a_w, t_b_w)
    required_2side = max(2 * base_side_w, max_team_w + pad_x * 2)
    side_w = (required_2side + 1) // 2
...
async def match_create(
    ...
    if ch in ongoing_bans:
        ...
        return await interaction.followup.send(
            "❌ Match active here; delete first.", ephemeral=True
        )
    ...
    img = create_ban_status_image(
        maps, ongoing_bans[ch], a, b, mode, flip_lbl, channel_decision[ch], cur_lbl
    )
...
async def ban_map(
    ...
    if ch not in ongoing_bans:
        return await interaction.followup.send(
            "❌ No match; use `/match_create`.", ephemeral=True
        )
    ...
    cur_lbl = None
    if not (len(combos_post) == 2 and combos_post[0][0] == combos_post[1][0]):
...
async def match_decide(
    ...
    if wl not in [r.name for r in interaction.user.roles]:
        return await interaction.response.send_message(
            "❌ Only flip winner.", ephemeral=True
        )

    channel_decision[ch] = choice
    match_turns[ch]      = (
        winner if choice == "ban"
        else ("team_b" if winner == "team_a" else "team_a")
    )
    save_state()
    ...
    await interaction.response.send_message(
        f"✅ Chose **{'First Ban' if choice=='ban' else 'Host'}**; "
        f"first ban: **{a_lbl if match_turns[ch]=='team_a' else b_lbl}**.",
        ephemeral=True
    )
...
async def match_delete(interaction: discord.Interaction):
    ...
    if ch not in ongoing_bans:
        return await interaction.response.send_message(
            "❌ No active match.", ephemeral=True
        )
