...
def create_ban_status_image(
    ...
    # load bold fonts if available
    try:
        row_font = ImageFont.truetype("arialbd.ttf", row_font_size)
    ...
    # calculate heights
    banner_h = header_font_size + 16
    h1 = header_font_size + 12
    h2 = header_font_size + 8
    row_h = row_font_size + 8
    ...
    draw.text((total_w//2, y + banner_h//2), banner_text, font=header_font, anchor="mm", fill="black")
    y += banner_h

    # Header row 1
    draw.rectangle([0, y, 2*sub_w, y + h1], fill=(200, 200, 200), outline="black")
    draw.text((sub_w, y + h1//2), team_a_label, font=header_font, anchor="mm", fill="black")
    draw.rectangle([2*sub_w, y, 2*sub_w + map_w, y + h1], fill=(200, 200, 200), outline="black")
    draw.text((2*sub_w + map_w//2, y + h1//2), "Maps", font=header_font, anchor="mm", fill="black")
    draw.rectangle([2*sub_w + map_w, y, total_w, y + h1], fill=(200, 200, 200), outline="black")
    draw.text((2*sub_w + map_w + sub_w, y + h1//2), team_b_label, font=header_font, anchor="mm", fill="black")
    y += h1
    ...
    for w, lab in zip(cols, labels):
        draw.rectangle([x, y, x + h2, y + h2], fill=(220, 220, 220), outline="black")
        if lab:
            draw.text((x + w//2, y + h2//2), lab, font=header_font, anchor="mm", fill="black")
        x += w
    y += h2
...
async def match_create(
    ...
    img       = create_ban_status_image(maps, ongoing_bans[ch], a, b, flip_lbl, dec_lbl, cur_lbl)
...
async def match_decide(
    ...
    if ch not in ongoing_bans or channel_flip.get(ch) is None:
        return await interaction.response.send_message("❌ No decision needed.", ephemeral=True)
    if channel_decision.get(ch) is not None:
        return await interaction.response.send_message("❌ Decision already made.", ephemeral=True)
    ...
    if winner_label not in [r.name for r in interaction.user.roles]:
        return await interaction.response.send_message("❌ Only the flip winner can decide.", ephemeral=True)

    channel_decision[ch] = choice
    match_turns[ch] = (winner_key if choice == "ban" else ("team_b" if winner_key == "team_a" else "team_a"))
    save_state()
    ...
    img           = create_ban_status_image(load_maplist(), ongoing_bans[ch], a_lbl, b_lbl, flip_lbl, dec_lbl, cur_lbl)
...
async def ban_map(
    ...
    ch = interaction.channel_id
    if ch not in ongoing_bans:
        return await interaction.followup.send("❌ No match here. Use `/match_create` first.", ephemeral=True)
    if channel_decision.get(ch) is None:
        return await interaction.followup.send("❌ Waiting for flip decision; use `/match_decide`.", ephemeral=True)

    team_key = match_turns.get(ch)
    if not team_key:
        return await interaction.followup.send("❌ Turn order not set.", ephemeral=True)
    role_name = channel_teams[ch][0] if team_key == "team_a" else channel_teams[ch][1]
    if role_name not in [r.name for r in interaction.user.roles]:
        return await interaction.followup.send("❌ It's not your turn to ban.", ephemeral=True)
    ...
    if len(combos_pre) == 2 and combos_pre[0][0] == combos_pre[1][0]:
        flip_lbl = channel_teams[ch][0] if channel_flip[ch] == "team_a" else channel_teams[ch][1]
        a_lbl, b_lbl = channel_teams[ch]
        img = create_ban_status_image(load_maplist(), ongoing_bans[ch], a_lbl, b_lbl, flip_lbl, "Done", None)
    ...
    cur_lbl  = None if len(combos_post) == 2 and combos_post[0][0] == combos_post[1][0] else channel_teams[ch][0 if match_turns[ch] == "team_a" else 1]
    img      = create_ban_status_image(load_maplist(), ongoing_bans[ch], channel_teams[ch][0], channel_teams[ch][1], flip_lbl, dec_lbl, cur_lbl)
...
async def match_delete(interaction: discord.Interaction):
    ch = interaction.channel_id
    if ch not in ongoing_bans:
        return await interaction.response.send_message("❌ No active match here.", ephemeral=True)
